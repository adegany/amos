...
| Automatic memory policy | `src/amos/service.py`, `src/amos/workers.py`, `src/amos/http_api.py`, `src/amos/cli.py`, `tests/test_amos_v1.py::test_automatic_memory_policy_distills_and_maintains_on_retrieval`, `tests/test_amos_v1.py::test_memory_policy_worker_force_runs_without_manual_maintenance` |
...
| Memory policy gate | Automatic scheduled policy ticks run deterministic distillation, SMP/steward maintenance, processor-pack distillation, derived-index refresh, packet-cache invalidation, persisted policy state, and `memory_policy_run` journal events |
...
| Observability gate | Memory/capacity health, projection lag, index freshness, retrieval outcomes, deletion residuals |
