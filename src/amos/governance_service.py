...
class GovernanceService:
    ...
    def ratify_proposal(
        ...
        *,
        ...
        if "self_ratification" not in self._capabilities(context):
            raise AccessDenied(f"{actor} lacks capability self_ratification")
        identity_ref = str(context.get("identity_ref") or "").strip()
        if not identity_ref:
            raise AccessDenied("self-ratification requires authorization_context.identity_ref")
        ...
        with self.store.transaction() as conn:
            ...
            for ref in covenant_refs:
                ...
                if covenant_scope and covenant_scope != proposal_scope:
                    raise ValidationError(
                        f"constitutional guidance scope does not match proposal: {ref}"
                    )
